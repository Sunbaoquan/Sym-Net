...
from util import config, transform, transform_tri
...
def main():
    ...
    logger = get_logger(args.save_path + '/{} train.log'.format(time.strftime('%Y-%m-%d %H-%M-%S', time.localtime())))
    ...
    if args.manual_seed is not None:
        ...
        logger.info("INFO: Seed {}".format(args.manual_seed))
    ...
    train_data = SemData(split=args.split, shot=args.shot, data_root=args.data_root, base_data_root=args.base_data_root,
    ...
    if args.evaluate:
        ...
        val_data = SemData(split=args.split, shot=args.shot, data_root=args.data_root,
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.fix_random_seed_val:
            setup_seed(args.manual_seed + epoch, deterministic=args.seed_deterministic)
        ...
        # save model for <resuming> -- per epoch
        if (epoch % args.save_freq == 0) and (epoch > 0) and main_process():
            pth_name = args.save_path + '/epoch_{}.pth'.format(epoch)
            logger.info('Saving checkpoint to: ' + pth_name)
        ...
        if args.evaluate and epoch % 1 == 0:
            ...
            if mIoU > max_iou:
                ...
                if os.path.exists(filename):
                    os.remove(filename)
                if args.shot == 1:
                    filename = args.save_path + '/train_epoch_' + str(epoch) + '_{:.4f}'.format(max_iou) + '.pth'
                else:
                    filename = args.save_path + '/train{}_epoch_'.format(args.shot) + str(epoch) + '_{:.4f}'.format(
                        max_iou) + '.pth'
                if main_process():
    ...
    if main_process():
        ...
        logger.info('\n<<<<<<<<<<<<<<<<<<<<<<<<<<<<<  Final Best Result   <<<<<<<<<<<<<<<<<<<<<<<<<<<<<')
        logger.info('Lite-FENet' + '\t Fold:{} \t Best_mIoU:{}'.format(args.split, max_iou))
...
def train(train_loader, val_loader, model, optimizer, epoch):
    global filename, max_iou, keep_epoch

    batch_time = AverageMeter()
    data_time = AverageMeter()
    main_loss_meter = AverageMeter()  # main loss
    aux_loss_meter1 = AverageMeter()  # aux loss 1
    aux_loss_meter2 = AverageMeter()  # aux loss 2
    aux_loss_meter3 = AverageMeter()  # aux loss 3
    aux_loss_meter4 = AverageMeter()  # aux loss 4
    ...
    for i, (input, target, target_b, s_input, s_mask, subcls, class_chosen) in enumerate(train_loader):

        data_time.update(time.time() - end)
        current_iter = epoch * len(train_loader) + i + 1

        poly_learning_rate(optimizer, args.base_lr, current_iter, max_iter, power=args.power, index_split=-1,
        ...
        output, main_loss, aux_loss1, aux_loss2, aux_loss3, aux_loss4 = model(s_x=s_input, s_y=s_mask, x=input,
                                                                              y=target, y_b=target_b, cat_idx=subcls,
                                                                              class_chosen=class_chosen)
        loss = main_loss + args.aux_weight * aux_loss1 + args.aux_weight * aux_loss2 + args.aux_weight * aux_loss3 + args.aux_weight * aux_loss4
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
        ...
        aux_loss_meter4.update(aux_loss4.item(), n)
        ...
        if (i + 1) % args.print_freq == 0 and main_process():
            logger.info('Epoch: [{}/{}][{}/{}] '
                        ...
                        'AuxLoss4 {aux_loss_meter4.val:.4f} '
                        ...
                        'Accuracy {accuracy:.4f}.'.format(epoch + 1, args.epochs, i + 1, len(train_loader),
                                                          ...
                                                          aux_loss_meter4=aux_loss_meter4,
            ...
            if args.viz:
                ...
                writer.add_scalar('loss_train_aux3', aux_loss_meter3.val, current_iter)
                writer.add_scalar('loss_train_aux4', aux_loss_meter4.val, current_iter)

        # -----------------------  SubEpoch VAL  -----------------------
        if args.evaluate and args.SubEpoch_val and (args.epochs <= 50 and epoch % 1 == 0 and epoch > 0) and (
            ...
            if mIoU > max_iou:
                ...
                if os.path.exists(filename):
                    os.remove(filename)
                if args.shot == 1:
                    filename = args.save_path + '/train_epoch_' + str(epoch - 0.5) + '_{:.4f}'.format(max_iou) + '.pth'
                else:
                    filename = args.save_path + '/train{}_epoch_'.format(args.shot) + str(
                        epoch - 0.5) + '_{:.4f}'.format(max_iou) + '.pth'
                if main_process():
                    logger.info('Saving checkpoint to: ' + filename)
                    torch.save(
                        {'epoch': epoch - 0.5,
    ...
    iou_class = intersection_meter.sum / (union_meter.sum + 1e-10)  # FB-IoU[0:, 1:]
    accuracy_class = intersection_meter.sum / (target_meter.sum + 1e-10)  # acc[0:, 1:]
    mIoU = np.mean(iou_class)  # FB-IoU
    mAcc = np.mean(accuracy_class)  # mAcc
...
def validate(val_loader, model):
    ...
    for e in range(db_epoch):
        for i, (input, target, target_b, s_input, s_mask, subcls, ori_label, ori_label_b, class_chosen) in enumerate(
            ...
            start_time = time.time()
            with torch.no_grad():
                output, meta_out, base_out = model(s_x=s_input, s_y=s_mask, x=input, y=target, y_b=target_b,
                                                   cat_idx=subcls,
                                                   class_chosen=class_chosen)
            ...
            if args.ori_resize:  # 真值转化为方形
                ...
                backmask_b = torch.ones(ori_label.size(0), longerside, longerside).cuda() * 255
    ...
    mIoU = np.mean(iou_class)  # final FB-mIoU
    mIoU_m = np.mean(iou_class_m)  # mata FB-mIoU
    ...
    class_miou = class_miou * 1.0 / len(class_intersection_meter)  # final mIoU
    class_miou_m = class_miou_m * 1.0 / len(class_intersection_meter)  # meta mIoU
    ...
    if main_process():
        ...
        for i in range(split_gap * 3):
            if class_target_meter_b[i] == 0:
                logger.info('Class_{} Result: iou(base) None.'.format(i + 1 + split_gap))
            else:
                logger.info('Class_{} Result: iou(base) {:.4f}.'.format(i + 1 + split_gap, class_iou_class_b[i]))
        logger.info('<===========FBIoU===========>')
        logger.info('FBIoU---Val result: FBIoU(final) {:.4f}.'.format(mIoU))
        logger.info('FBIoU---Val result: FBIoU(mata) {:.4f}.'.format(mIoU_m))
        for i in range(args.classes):
