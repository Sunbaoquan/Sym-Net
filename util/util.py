...
def poly_learning_rate(optimizer, base_lr, curr_iter, max_iter, power=0.9, index_split=-1, scale_lr=10.,
    ...
    """poly learning rate policy，事实是config的lr*10"""
    ...
    for index, param_group in enumerate(optimizer.param_groups):
        if index <= index_split:
            ...
            param_group['lr'] = lr * scale_lr   # 10 x LR
    if curr_iter % 100 == 0:
        print('Base_lr: {:.6f}, Curr_lr: {:.6f}, Param_lr:{:.6f}, Warmup: {}.'.format(base_lr, lr, optimizer.param_groups[0]['lr'], (warmup and curr_iter < warmup_step)))



def setup_seed(seed=321, deterministic=False):
    """setup random seed"""
    if deterministic:
