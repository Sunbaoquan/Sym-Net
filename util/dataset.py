...
def make_dataset(split=0, data_root=None, data_list=None, sub_list=None, filter_intersection=False):
    ...
    print("Checking <image label> pair split_{} list done! ".format(split))
    print("{} <images label> pairs after processing!".format(len(image_label_list)))
    return image_label_list, sub_class_file_list


class SemData(Dataset):
    def __init__(self, split=3, shot=1, data_root=None, base_data_root=None, data_list=None,
                 data_set=None, use_split_coco=False, mode='train',
                 transform=None, transform_tri=None, ann_type='mask'):
        ...
        if data_set == 'pascal':  # use pascal 20
            print("Use PASCAL-5i dataset...")
            ...
            if use_split_coco:
                print('INFO: using COCO with split partition (FWB)')
                ...
                print('INFO: using COCO split serial (PANet)')
                self.class_list = list(range(1, 81))
                if self.split == 3:
        ...
        print('sub_list(train): ', self.sub_list)
        print('sub_val_list(val): ', self.sub_val_list)
        ...
        fss_data_list_path = fss_list_root + 'data_list_{}.txt'.format(split)   # data_list
        fss_sub_class_file_list_path = fss_list_root + 'sub_class_file_list_{}.txt'.format(split)  # sub_class_file_list
    ...
    def __getitem__(self, index):
        ...
        # return
        if self.mode == 'train':
            return image, label, label_b, s_x, s_y, subcls_list, class_chosen
        else:
